...
function AnalyzeContent() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const res = await fetch("http://127.0.0.1:8001/api/key-status");
        if (res.ok) {
  ...
  const handleModeChange = async (newMode: string) => {
    ...
    if (status === "success") {
      ...
      try {
        const res = await fetch("http://127.0.0.1:8001/api/resynthesize", {
          method: "POST",
            ...
            if (dataStr) {
              try {
                ...
                if (parsed.log) {
                   ...
                   fetch(`http://127.0.0.1:8001/api/drift?repo_url=${encodeURIComponent(url)}`)
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    setQaError("");
    try {
      const res = await fetch("http://127.0.0.1:8001/api/qa", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleGeneratePath = async () => {
    ...
    setPathError("");
    try {
      const res = await fetch("http://127.0.0.1:8001/api/onboard", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDraft = async () => {
    ...
    setDraftError("");
    try {
      const res = await fetch("http://127.0.0.1:8001/api/draft", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
