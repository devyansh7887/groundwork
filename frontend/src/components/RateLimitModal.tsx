...
export function RateLimitModal({ isOpen, onClose, onSubmit }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="bg-[#161b22] border border-[#da3633]/40 rounded-xl w-full max-w-lg overflow-hidden">
        
        {/* Header */}
        <div className="p-5 border-b border-[#30363d] flex items-start gap-3 bg-[#da3633]">
          <div className="w-9 h-9 rounded-full bg-[#da3633] flex items-center justify-center flex-shrink-0 mt-0.5">
            <span className="w-5 h-5 text-[#ff7b72]">[!]</span>
          </div>
          <div>
            <h2 className="font-space text-lg font-bold text-[#c9d1d9]">API Rate Limit Hit</h2>
          ...
          <div className="px-5 pt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {keyStatuses.map(k => {
                ...
                return (
                  <div key={k.id} className={`rounded-md p-3 border ${isReady ? 'border-[#238636]/40 bg-[#238636]' : 'border-[#da3633]/30 bg-[#da3633]'}`}>
                    <div className="flex items-center justify-between text-xs font-jetbrains mb-2">
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="relative mb-3">
            ...
            <input
              ...
              className="w-full bg-[#0d1117] border border-[#30363d] rounded-md py-2.5 pl-10 pr-3 text-[#c9d1d9] font-jetbrains text-sm focus:outline-none focus:border-[#58a6ff] focus:ring-1 focus:ring-[#58a6ff]"
