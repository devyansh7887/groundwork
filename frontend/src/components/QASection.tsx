...
import { Search, Terminal, XCircle, Loader2, CheckCircle, HelpCircle, Maximize2, Minimize2 } from "lucide-react";
...
export function QASection({ qaHistory, asking, qaError, question, setQuestion, setQaError, handleAsk }: Props) {
  ...
  const content = (
    <div className={`flex-1 w-full flex flex-col bg-[#161b22] border border-[#30363d] rounded-md overflow-hidden shadow-sm ${isExpanded ? 'h-full border-none' : 'h-[700px] mt-2'}`}>
      <div className="p-5 border-b border-[#30363d] bg-[#161b22] flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-[#c9d1d9] flex items-center"><Terminal className="w-4 h-4 mr-2 text-[#8b949e]" /> Grounded Q&A</h3>
          <p className="text-xs font-mono text-[#8b949e] mt-1">Every claim verified via AST graph.</p>
        </div>
        <button 
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-[#0d1117]">
        {qaHistory.length === 0 ? (
          <div className="text-sm font-mono text-[#8b949e] text-center mt-32 px-4">
            <Terminal className="w-10 h-10 text-[#30363d] mx-auto mb-4" />
            &gt;_ Ask a question about the architecture.<br/>Responses are fact-checked concurrently.
          </div>
        ) : (
          qaHistory.map((qa, i) => (
            <div key={i} className="space-y-4">
              <div className="bg-[#161b22] border border-[#30363d] p-4 rounded-md self-end ml-8 shadow-sm">
                <p className="text-sm md:text-base text-[#c9d1d9] font-medium">{qa.q}</p>
              </div>
              <div className="bg-[#161b22] border border-[#30363d] border-l-4 border-l-[#58a6ff] p-5 rounded-md mr-4 shadow-sm">
      ...
      <div className="p-5 border-t border-[#30363d] bg-[#161b22]">
        {qaError && (
          <div className="mb-3 bg-[#da3633]/10 border border-[#da3633]/40 text-[#ff7b72] text-xs font-mono p-3 rounded flex justify-between items-center">
            <span>{qaError}</span>
            <button onClick={() => setQaError("")}><XCircle className="w-3.5 h-3.5" /></button>
          </div>
        )}
        <form onSubmit={handleAsk} className="relative">
          <input
            ...
            className="w-full pl-4 pr-12 py-4 bg-[#0d1117] border border-[#30363d] rounded-md focus:ring-1 focus:ring-[#58a6ff] focus:border-[#58a6ff] text-base text-[#c9d1d9] placeholder-[#8b949e] font-mono outline-none shadow-inner transition-shadow"
          />
